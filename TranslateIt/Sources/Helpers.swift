...
public func setupLanguageTranslatorV3() -> LanguageTranslator {
    ...
    let version = "2020-04-05"
    let apiKey = IAMAuthenticator(apiKey: WCredentials.LanguageTranslatorV3APIKey)

    let languageTranslator = LanguageTranslator(version: version, authenticator: apiKey)

    // Set the URL for the service endpoint if needed

    languageTranslator.serviceURL = WCredentials.LanguageTranslatorV3URL
